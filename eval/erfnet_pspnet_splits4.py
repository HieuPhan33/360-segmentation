...
class PSPDec(nn.Module):
    ...
    def __init__(self, in_features, out_features, downsize, upsize=(64, 128), use_new_padding=False):
        ...
        self.use_new_padding = use_new_padding#False
...
class Decoder(nn.Module):
    def __init__(self, num_classes, use_new_padding=False, size=(512,1024)):
        super().__init__()
        self.use_new_padding = use_new_padding
        self.size=size
        upsize = (size[0]//8, size[1]//8)
        pooling = upsize
        
        self.layer5a = PSPDec(128, 32, pooling, upsize, use_new_padding=self.use_new_padding)
        self.layer5b = PSPDec(128, 32, (pooling[0]//2, pooling[1]//2), upsize, use_new_padding=self.use_new_padding)
        self.layer5c = PSPDec(128, 32, (pooling[0]//4, pooling[1]//4), upsize, use_new_padding=self.use_new_padding)
        self.layer5d = PSPDec(128, 32, (pooling[0]//8, pooling[1]//8), upsize, use_new_padding=self.use_new_padding)
    ...
    def forward(self, x):
        ...
        x = self.pooling(x) # B,C*5, H, W
        x = self.final(x)
        
        if self.use_new_padding:
            left = x[:,:,:,-1:] # Get left strip
            right = x[:,:,:,:1] # Get right strip
            # Size of one strip after upsampling = upsample factor
            factor = self.size[1] // x.size(3) 
            x = torch.cat([left,x,right],3) # Pad L to R, L to L -> full context
            x = F.upsample(x,size=(self.size[0],self.size[1]+factor*2), mode='bilinear')
            x = x[:,:,:,factor:-factor]

        return F.upsample(x, size=(self.size[0], self.size[1]*2), mode='bilinear')


class Net(nn.Module):
    def __init__(self, num_classes, encoder=None, size=(512,1024)):  # use encoder to pass pretrained encoder
        super().__init__()
        ...
        self.decoder = Decoder(num_classes, use_new_padding=True, size=size)
